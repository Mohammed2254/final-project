# Stage 5 — Project Closure · Deliverables

**Project:** Farah — Wedding Planning Platform
**Repository:** https://github.com/Mohammed2254/final-project
**Live at:** https://farah-592g.onrender.com

---

## Deliverables

| Deliverable | Status | Link |
|---|---|---|
| Results summary | ✅ Done | [`01_results_summary.md`](01_results_summary.md) |
| Lessons learned | ✅ Done | [`02_lessons_learned.md`](02_lessons_learned.md) |
| Team retrospective (Stage 5 meeting) | ✅ Done | [`02_lessons_learned.md` — final section](02_lessons_learned.md#team-retrospective-stage-5) |
| Presentation slide deck | ✅ Done | [`farah_presentation.pdf`](farah_presentation.pdf) |
| Live presentation | ⬜ Scheduled | — |

## What's in the two documents already written

**[`01_results_summary.md`](01_results_summary.md)** — the three
Charter objectives lined up against what actually shipped, with a link
to evidence for each; the project's final numbers (commits, tests, bugs,
endpoints); what was deliberately left out of scope; and an honest note
on how the real timeline compared to the planned one.

**[`02_lessons_learned.md`](02_lessons_learned.md)** — builds on Stage
4's own retrospective rather than repeating it. Covers the whole project
lifecycle: what worked from Stage 1 onward, what didn't and why, a
direct check of every risk the Charter predicted back in Stage 2 against
what actually happened, and concrete changes for next time.

The retrospective section at the end of that second document is the
team's own discussion, held separately from the technical retrospective
and written from what was actually said in it.

## What's left

**Deliver the live presentation.** Everything written is done; the deck
is built and the product is deployed and verified.
